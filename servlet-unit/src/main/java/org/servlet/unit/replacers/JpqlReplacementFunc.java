...
public class JpqlReplacementFunc implements ReplaceFunction {
	...
	private void init() {
		try {
			EntityManagerFactory em = appContext
	...
	public Pattern getPattern() {
		return Pattern.compile("jpql\\{(.*?)\\}(\\[([0-9]*)\\])?",
				Pattern.MULTILINE);
	}
	...
	public String replace(Matcher matcher, TestCase replacer) {
		...
		int index = (matcher.group(2) != null) ? Integer.parseInt(matcher
				.group(2)) : 0;

		final String sql = matcher.group(1);

		try {
			Object obj = entityManager.createQuery(sql).setFirstResult(index)
